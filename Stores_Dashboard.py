    ...
    try:
        ...
        with col1:
            if df_orders is not None and not df_orders.empty:
                ...
                st.markdown(f"<h1 style='display: inline-block;'>Unique Orders {tooltip_html}</h1>",unsafe_allow_html=True)
                st.markdown(f"""
                    <div class="card">
            ...
            if df_orders is not None and not df_orders.empty:
                ...
                st.markdown(f"<h1 style='display: inline-block;'>Preview Filtered Order Data {tooltip_html}</h1>",unsafe_allow_html=True)
        ...
        st.markdown(f"<h3 style='font-size: 30px; color: red; text-align: center;'><b>Dataset Currently unavaialbe</b></h3>",
            unsafe_allow_html=True)
    ...
    try:
        ...
        try:
            if (df_products is not None and not df_products.empty)and (df_orders is not None and not df_orders.empty):
                ...
                unsold_products_grouped['Product_Published_At'] = unsold_products_grouped['Product_Published_At'].str.split("T").str[0]
